...
//    }
//
//    private Matrix getY(List<TeaInterested> teaList) {
//        int m = teaList.size();
//        Matrix matrix = Matrix.Factory.zeros(m, 1);
//        for (int i = 0; i < matrix.getSize()[0]; ++i) {
//            for (int j = 0; j < matrix.getSize()[1]; ++j) {
//                matrix.setAsDouble(teaList.get(i).getPrice(), i, j);
//            }
//        }
//        return matrix;
//    }
//
//    public Matrix getParamByMonthMatrix(List<TeaInterested> teaList, Double regFactor) {
...
//        return (((getX(teaList).transpose().mtimes(getX(teaList))).plus(identity.times(regFactor)).inv()).mtimes(getX(teaList).transpose())).mtimes(getY(teaList));
//    }
//
...
//    }
//
//    public Matrix getParamByMonthByGradient(List<TeaInterested> teaList, Double regFactor, Double step, int iteration) {
//        int m = teaList.size();
...
//                              Double theta4, Double theta5, Double theta6, Double theta7, Double theta8, Double theta9,Double theta10) {
//        Double result = 0.0;
//        if (index == 0) {
...
//                        theta8 * tea.getByOrder(8) + theta9 * tea.getByOrder(9)+theta10*tea.getByOrder(10) - tea.getPrice();
//            }
//        } else {
//            for (TeaInterested tea : teaList) {
//                result = result + (theta0 + theta1 * tea.getByOrder(1) + theta2 * tea.getByOrder(2) + theta3 * tea.getByOrder(3) +
//                        theta4 * tea.getByOrder(4) + theta5 * tea.getByOrder(5) + theta6 * tea.getByOrder(6) + theta7 * tea.getByOrder(7) +
//                        theta8 * tea.getByOrder(8) + theta9 * tea.getByOrder(9)+theta10*tea.getByOrder(10) - tea.getPrice()) * tea.getByOrder(index);
//            }
//        }
//        return result;
//    }
